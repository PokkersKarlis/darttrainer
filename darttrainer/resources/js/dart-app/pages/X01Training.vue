...
import { Training } from '../api/client.js';
import X01SoloProtocol from '../components/X01SoloProtocol.vue';
...
const activeMultiplier = ref(1); // 1=S 2=D 3=T

const form = reactive({ variant: 501, in_mode: 'straight', out_mode: 'double' });
const dartInput = reactive({ darts: [] });
const feedback = ref(null); // { bust, checkout, scored }

const protocolOnlyState = ref(null);
const protocolLoading = ref(false);
const protocolError = ref(false);

const showProtocolQuery = computed(() => {
...
watch(() => route.query.protocol, syncProtocolQuery);
