const LobbyPage = {
  setup() {
    ...
    const lobbyShellLocked = Vue.computed(
      () => checkingSetupMatch.value && !room.value,
    );
    ...
    function startRoomPoll() {
      ...
      roomPoll = setInterval(async () => {
        ...
        try {
          ...
          room.value = data;
          if (data.match_id) { stopRoomPoll(); router.push(`/game/${data.match_id}`); }
        } catch (_) {}
      }, 2000);
    }
    ...
    async function joinRoom() {
      ...
      } catch (e) { error.value = e.response?.data?.error || 'Kods nav atrasts.'; }
      finally { loading.value = false; }
    }
    ...
    async function confirmStartGame() {
      ...
      try {
        ...
        stopRoomPoll();
        router.push(`/game/${data.match_id}`);
      } catch (e) { error.value = e.response?.data?.error || 'Kļūda startējot.'; }
      finally { loading.value = false; }
    }

    return {
      auth, tab, loading, checkingActive, error, room,
      isHost, hostPlayer, t,
      showOrderModal, orderMethod, playerOrder, ORDER_METHODS, ORDER_METHOD_KEYS,
      createForm, joinForm,
      createWizardStep, goWizardNext, goWizardBack, openCreateTab,
      createRoom, joinRoom, leaveRoom, copyCode,
      openOrderModal, setOrderMethod, moveUp, moveDown, confirmStartGame,
      BO_LEG_OPTIONS, SET_COUNT_OPTIONS,
      friendList, presetList, selectedFriendIds, selectedPresetIds, adHocGuests,
      newAdHocGuest, localRosterCount,
      filteredFriendList, filteredPresetList,
      localPickerTab, friendSearch, presetSearch, saveGuestToLibrary, guestPickerBusy,
      loadFriendsAndPresets, toggleFriendId, togglePresetId, addAdHocGuest, removeAdHoc,
      submitAddGuestFromPicker, deletePreset, rosterAtCap,
      localRosterDisplay, removeFromLocalRoster,
      blockedRoomForType, refreshBlockedRoomForType, continueExistingTypeRoom,
      blockedRoomSummaryLine,
      checkingSetupMatch, lobbyShellLocked,
      showLobbyTip,
    };
  },
    ...
    <div class="flex flex-1 min-h-0 w-full max-w-lg lg:max-w-4xl mx-auto px-2 sm:px-3 py-1 sm:py-1.5 flex flex-col gap-1 sm:gap-1.5 min-h-0">
      ...
      <div class="shrink-0 flex items-start justify-between gap-2">
        <div class="min-w-0 text-left flex-1">
          <h1 class="text-base sm:text-lg font-black text-white tracking-tight leading-tight">{{ t('lobby.title') }}</h1>
          <p class="text-slate-600 text-[10px] sm:text-xs mt-0.5 line-clamp-1 sm:line-clamp-none">{{ t('lobby.subtitle') }}</p>
        </div>
...
aaaaaaaaaaaaaaaaaaaaaaaa
        ...
        <div v-else class="flex flex-1 min-h-0 min-w-0 flex-col gap-1 overflow-hidden">
          ...
          <div v-if="tab === 'create'" key="c"
            ...
            <div class="shrink-0 flex gap-2 lg:gap-3">
              <button type="button" @click="createForm.game_type = 'x01'"
                      class="flex-1 flex items-center gap-2.5 p-2.5 rounded-2xl border-2 transition-all active:scale-[0.99] ring-offset-2 ring-offset-[#060d18] lg:min-h-[5.75rem] lg:p-4 lg:gap-3"
                      :class="createForm.game_type === 'x01'
                        ? 'border-amber-500/90 bg-gradient-to-br from-amber-500/12 to-slate-900/50 ring-2 ring-amber-500/30'
                        : 'border-slate-700/70 bg-slate-800/45 hover:border-slate-600 ring-2 ring-transparent'">
                <span class="text-2xl leading-none drop-shadow-sm lg:text-3xl">🎯</span>
                <div class="text-left min-w-0">
                  <div class="font-black text-white text-sm tracking-tight lg:text-base">X01</div>
                  <div class="text-[9px] text-slate-500 leading-none hidden min-[400px]:block lg:text-[10px] lg:mt-1">501 · 301</div>
                </div>
              </button>
              <button type="button" @click="createForm.game_type = 'cricket'"
                      class="flex-1 flex items-center gap-2.5 p-2.5 rounded-2xl border-2 transition-all active:scale-[0.99] ring-offset-2 ring-offset-[#060d18] lg:min-h-[5.75rem] lg:p-4 lg:gap-3"
                      :class="createForm.game_type === 'cricket'
                        ? 'border-emerald-500/90 bg-gradient-to-br from-emerald-500/12 to-slate-900/50 ring-2 ring-emerald-500/30'
                        : 'border-slate-700/70 bg-slate-800/45 hover:border-slate-600 ring-2 ring-transparent'">
                <span class="text-2xl leading-none drop-shadow-sm lg:text-3xl">🏏</span>
                <div class="text-left min-w-0">
                  <div class="font-black text-white text-sm tracking-tight lg:text-base">Cricket</div>
                  <div class="text-[9px] text-slate-500 leading-none hidden sm:block lg:text-[10px] lg:mt-1">15–20 + Bull</div>
                </div>
              </button>
            </div>

            <template v-if="createForm.game_type === 'x01'">
            ...
            <template v-if="createForm.game_type === 'cricket'">
