const StatisticsPage = {
  setup() {
    ...
    const leaderboardCricket = Vue.ref([]);
    const leaderboardSolo   = Vue.ref([]);
    const protocolMatches   = Vue.ref([]);
    const protocolLoading   = Vue.ref(false);
    const soloProtocols     = Vue.ref([]);
    const soloProtocolLoading = Vue.ref(false);

    const open = Vue.reactive({
      cricket: true,
      x01: true,
      protocol: false,
      lbCricket: true,
    ...
    async function loadLeaderboards() {
      try {
        const [cr, xs] = await Promise.all([
          Stats.leaderboard('cricket'),
          Stats.leaderboard('x01_solo'),
        ]);
