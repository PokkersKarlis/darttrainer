...
const MatchReport = {
  ...
  setup(props, { emit }) {
    ...
    const payload = Vue.ref(null);

    const isCricket = Vue.computed(() => payload.value?.match?.game_type === 'cricket');
    const isX01     = Vue.computed(() => payload.value?.match?.game_type === 'x01');
    const report    = Vue.computed(() => payload.value?.report ?? null);
    const meta      = Vue.computed(() => report.value?.meta ?? {});
    const rows      = Vue.computed(() => report.value?.player_rows ?? []);
    const agp       = Vue.computed(() => report.value?.agp ?? null);
    const legRows   = Vue.computed(() => report.value?.leg_rows ?? []);
