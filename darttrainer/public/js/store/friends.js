function dartFriendMsg(key, name) {
  ...
  let tpl = window.DART_I18N?.[loc]?.friends?.[key];
  if (!tpl) tpl = window.DART_I18N?.en?.friends?.[key];
  if (!tpl) return key;
  return String(tpl).replace(/\{name\}/g, name);
}

const useFriendsStore = Pinia.defineStore('friends', {
  state: () => ({
    incoming: [],
    outgoing: [],
    /** friendRequest id -> last known status (for requester toasts) */
    outgoingStatusSeen: {},
    pollTimer: null,
    modalOpen: false,
    busyId: null,
    ...
    checkOutgoingToasts(items) {
      ...
      for (const row of items) {
        ...
        if (prev === 'pending' && row.status === 'accepted') {
          window._dartToast?.(
            dartFriendMsg('toastTheyAccepted', row.user?.name ?? '?'),
            'success'
          );
        }
        if (prev === 'pending' && row.status === 'rejected') {
          window._dartToast?.(
            dartFriendMsg('toastTheyRejected', row.user?.name ?? '?'),
            'error'
          );
        }
